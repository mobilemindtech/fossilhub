...
proc packer::main {argc argv} {
    ...
    if {$argc > 1} {
        if {[lindex $argv 1] == "-with-config"} {
            set build_with_config [lindex $argv 2]

            if {![file exists $build_with_config]} {
                puts "::> config file $build_with_config not exists"
                exit 1
            }
            
            set configs [read_configs $build_with_config]
        }
    }
...
# read app configs
#
proc packer::read_configs {{filepath ""}} {
    set build_file  [expr {$filepath == "" ? "./build.yaml" : $filepath}] 
    set configs {}
    if {[file exists $build_file]} {        
    ...
    return -code error "config $build_file file not found"
}
...
proc packer::load_lib_from_git {deps uri} {
	...
	if {[file exists $deps/$dirname]} {
        ...
        set custom_build $deps/$dirname/build.yaml
        if {[file exists $custom_build]} {

            puts "::> execute build for $dirname"

            main 3 [list build -with-config $deps/$dirname/build.yaml]
        }
	}
